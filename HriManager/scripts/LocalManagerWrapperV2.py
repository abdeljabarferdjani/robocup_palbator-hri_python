...
class LocalManagerWrapperPalbator:
    GOAL = "goal"
    RESULT = "result"
    ...
    def __del__(self):
        ...
        rospy.loginfo("LMW : ABOUT TO BE DESTROYED")
    ...
    def __init__(self):#, ip_address, tcp_port, prefix):
        ...
        self.sub_feedback_action_server=rospy.Subscriber("action_GmToHri/feedback",GmToHriFeedback,self.handle_feedback)

        self.restart_hri_mode='OFF'
        try:
            self.client_action_GmToHri.cancel_all_goals()
        except Exception as ex:
            rospy.logwarn("LOG EXCEPTION : "+str(ex))        # self._prefix = prefix
        ...
        # while not self._configure_palbator(ip_address, tcp_port) and not rospy.is_shutdown():
        #     time.sleep(0.5)

        # TODO Add Heartbeat

        # self.pub_restart_mode.publish('OFF')
        rospy.loginfo("{class_name}: Configuration finished.".format(class_name=self.__class__))

    # def handle_restart_mode(self,req):
    #     self.restart_hri_mode=req.data
    
    def handle_feedback(self,req):

        rospy.loginfo("LWM : FEEDBACK : "+str(req.feedback.Gm_To_Hri_feedback))



    def _configure_palbator(self, ip, port):
        ...
        # return True
        pass
        ####### CHECKER SI BESOIN DE LIAISON TCP IP AVEC PALBATOR OU SI EXECUTION EN LOCAL
    ...
    def _palbator_callback(self, payload):
        rospy.loginfo("{class_name}: Palbator callback called. Payload: {payload}".format(
    ...
    def _execute_request(self, action, payload, timeout):#,wait_for_HRI):
        start_time = time.time()
        rospy.loginfo("LMW : REQUEST RECEIVED FROM SCENARIO")
        # rospy.loginfo(
        ...
        try:
            ...
            #         return GoalStatus.PREEMPTED, None
            #     time.sleep(0.1)
            # if self.restart_hri_mode=='OFF':
            rospy.loginfo("LMW : WAITING FOR ACTION RESULT ")
            # if wait_for_HRI==True:
