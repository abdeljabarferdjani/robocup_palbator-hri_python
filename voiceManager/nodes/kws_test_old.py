...
class KWSDetection(object):
    ...
    def __init__(self):
        ...
        if rospy.has_param(_lm_param):
            self.lm_dir=rospy.get_param(_lm_param)
        ...
        if rospy.has_param(_lm_param)==False:
    ...
    def start_recognizer(self):
        ...
        if self.lm_dir != None:
            config.set_string('-lm',self.lm_dir)
        else:
            if self._list:
    ...
    def process_audio(self, data):
        ...
        if not self.stop_output:
            ...
            if self.decoder.hyp() != None:
                rospy.loginfo([(seg.word, seg.prob)for seg in self.decoder.seg()])
